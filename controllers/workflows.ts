    ...
    if (!request.hasBody) {
        ...
        try {
            ...
            if (workflow.metadata) {
                ...
                const res = await dbClient.queryArray
...
// deno-lint-ignore no-explicit-any
const getWorkflowProgress = async ({ response, params }: { response: any, params: any }) => { 
    if (params && params.requestId) {
        const dbClient = await dbPool.connect();   
        const result = await dbClient.queryObject(`SELECT running_task_id, status, task_status, task_name, failed_count FROM public.workflows WHERE request_id = '${params.requestId}'`);
        const totalStatuses = await dbClient.queryObject(`SELECT status, COUNT (status)
        FROM public.workflows WHERE status='queued' OR status='started' GROUP BY status`);
        await dbClient.release();
        let totalQueued: any = totalStatuses.rows.find(element => element.status === 'queued');
        totalQueued = totalQueued ? totalQueued.count : 0
        let totalStarted: any = totalStatuses.rows.find(element => element.status === 'started');
        totalStarted = totalStarted ? totalStarted.count : 0
        // deno-lint-ignore no-explicit-any
        const workflow: any = result.rows[0]; 
        if (workflow) {
            const taskId: number = workflow.running_task_id ? workflow.running_task_id : 0
...
// deno-lint-ignore no-explicit-any
const getResult = async ({ response, params }: { response: any, params: any }) => { 
    if (params && params.requestId) {
        const dbClient = await dbPool.connect();   
        const result = await dbClient.queryObject(`SELECT result_location, status FROM public.workflows WHERE request_id = '${params.requestId}'`);
        await dbClient.release();
        // deno-lint-ignore no-explicit-any
        const workflow: any = result.rows[0]; 
        if (workflow) {
